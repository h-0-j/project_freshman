...
##CT,CMx,CMy식 책에서 찾기. 
        CT += np.sum(dT)/(0.5*rho*U_tip**2*np.pi*R**2)
        CMx += np.sum(dT*r_R*np.sin(psi_vals))*dpsi/(0.5*rho*U_tip**2*np.pi*R**3)
        CMy += np.sum(dT*r_R*np.cos(psi_vals))*dpsi/(0.5*rho*U_tip**2*np.pi*R**3)
...
def newton_trim(x0, target_CT, target_CMx, target_CMy, tol=1e-6, max_iter=50):
    ...
    for i in range(max_iter):
        ...
        print(f"[Iteration {i+1}] Residual: {res}")
...
# 초기 추정값
x_initial = np.array([np.radians(5.62), np.radians(0.64), np.radians(-4.84)])
...
print(f"Trimmed pitch angles:\n theta0={theta_0_trim:.3f}, theta1c={theta_1c_trim:.3f}, 1thetas={theta_1s_trim:.3f}")
