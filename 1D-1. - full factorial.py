...
# Full Factorial 데이터 생성
n_samples = 50
x_full = np.linspace(0, 1, n_samples).reshape(-1, 1)
y_full = f(x_full).ravel()
...
for fold_idx, (train_idx, test_idx) in enumerate(kf.split(x_full)):
    ...
    fig.suptitle(f"Fold {fold_idx + 1}", fontsize=14)
...
print(f"\n✅ 평균 Validation R² Score across folds: {mean_r2:.5f}")
