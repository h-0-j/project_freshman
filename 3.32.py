...
#왜 안될까 ? 중요한 물리현상이 덜 반영되었을 가능성 -> 식이 잘못된것이 들어갔거나 이상하다 이거지.
#3.31 + lambda_0 update (with stable lambda update)
#겁나 많이 돌아가서 + damping 추가
import numpy as np
...
def compute_aero_coefficients(x):
    ...
    for j, r_R in enumerate(r_vals):
        ...
        C_n_total[:, j] = dT / (0.5 * rho * U_eff**2 * area_section)
...
# 🔹 Contour plot
contour = ax.contourf(Psi, R_grid, Alpha_effective.T, levels = np.linspace(-10, 10, 21), cmap="gray")  # AoA 값 degree 변환
#plt.ylim(0.3, 0.6)
